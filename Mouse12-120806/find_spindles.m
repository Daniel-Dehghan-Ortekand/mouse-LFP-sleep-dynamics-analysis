function spindles = find_spindles(filtered, th_onset, varargin)
    ...
    newThrehold = ones(size(peak_z))*th_onset;%peak_z/3;
    % 2) Select spindles that need correction
    indices = (1:length(peak_z))'; %find(newThrehold>threshold); 
    ...
    st_i = Accumulate(id(in&belowThreshold),time(in&belowThreshold),'mode','max')+dt;
    if(length(st_i) < length(indices))
        start(indices(1:length(st_i))) = st_i;
    end


    % Candidate intervals for the new spindle stop = between the
    % current spindle peak and next spindle start
    if indices(1) > 1, candidateIntervals = [stop(indices) start(indices+1)];
    else, candidateIntervals = [stop(indices(1:end-1)) start(indices(2:end)); stop(indices(end)) time(end)]; end
    
    must_remove = diff(candidateIntervals,1,2)<0.1;
    indices(must_remove) = [];
    candidateIntervals(must_remove,:) = [];
    ...
    stop(indices) = Accumulate(id(in&belowThreshold),time(in&belowThreshold),'mode','min')+dt;
...
% 	            peak_t = peak_t(tooClose+i-1);
    ...
    spindles(sum(isnan(spindles),2) > 0, :) = [];
