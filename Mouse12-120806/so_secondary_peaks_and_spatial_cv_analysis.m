...
% different frequency bands (broadband 0.5-10 Hz vs. the original 0.5-4 Hz 
% filtered signal) and visualizes specific high/low heterogeneity events.
%
% INPUTS:
%   - thal_ch_norm_filtered_ChooseOneChannelFromEachShanks.mat: Filtered LFP 
%     data (one representative channel per shank, 0.5-4 Hz).
%   - daleyForOneChannnelOfEachShankBaseOnMaxPeak.mat: Precomputed inter-channel 
%     delay data.
...
for iii=1:size(refTime,2)
   ...
   % Find the index of the biggest peak
    ...
    biggestPeakAmplitude(iii) = peakAmplitudesRaw(maxPeakIndex);
