...
%
% INPUTS:
%   - thal_ch_norm_filtered_120806.mat: Pre-filtered LFP data.
%   - meansSignalForMethode2ToSeprate120806.mat: Population spindle signal.
%   - thal_spindles_ch[X].mat: Detected spindle events per channel.
%   - real_peaks[X].mat: Peak amplitude indices per channel.
%
% OUTPUTS:
%   - sepratedSpindles120806.mat: Structure containing shank-specific 
...
% end
%    save thal_ch_norm_filtered_120806.mat thal_ch_norm_filtered -v7.3
...
%       save (['thal_spindles_120806_ch' num2str(k) '.mat'],'thal_spindles') 
% end

%% SECTION three : find pick
% for m=1:ch_number
%    load  (['thal_spindles_120806_ch' num2str(m) '.mat'])
...
% 
%    end
%     save (['real_peaks_120806_' num2str(m) '.mat'],'real_peaks') 
% 
% 
% end 
...
% hold on
% for i=1:ch_number
%    load  (['thal_spindles_120806_ch' num2str(i) '.mat'])
...
% signal= Means;
% save meansSignalForMethode2ToSeprate120806    signal
% %
% figure
% hold on 
% for i=1:ch_number
...
load thal_ch_norm_filtered_120806.mat
load meansSignalForMethode2ToSeprate120806    signal
...
for i = 1:8
    ...
    for j = (i-1)*8+1:i*8
        ...
        load  (['thal_spindles_ch' num2str(ch_ind) '.mat']);
...
for k=1:size(mostSpinForEachCh,2)
    load  (['thal_spindles_ch' num2str(mostSpinForEachCh(1,k)) '.mat'])
    load meansSignalForMethode2ToSeprate120806    signal
...
for k=1:size(mostSpinForEachCh,2)
    ...
    locspin=[];
    locPeaks =[];
load (['real_peaks' num2str(mostSpinForEachCh(1,k)) '.mat'])
load  (['thal_spindles_ch' num2str(mostSpinForEachCh(1,k)) '.mat'])
...
sepratedSpindles120806.referencChannelForEachChannel = mostSpinForEachCh;

sepratedSpindles120806.referencChannel = mostSpinCh;
sepratedSpindles120806.localSPindles = LocSpinForEachRef;
sepratedSpindles120806.globalSPindles = globSpinForEachRef;
...
save_path = 'D:/daniel/Mouse20-120806/SpindlesInf';
