function delta = FindDelta(filtered, filtered_fast, fs,varargin)
...
% hold on
% plot(filtered_fast(:,1), y, '--b')
% plot(delta(:,1), z(round(delta(:,1)*fs),2), '>r', 'MarkerFaceColor', 'r')
% plot(delta(:,2), z(round(delta(:,2)*fs),2),'h' ,'color' ,'m', 'MarkerFaceColor', 'm','MarkerSize',8)
% plot(delta(:,3), z(round(delta(:,3)*fs),2), '<c', 'MarkerFaceColor', 'c')
% plot([z(1,1) z(end,1)],[highPeak highPeak], 'LineWidth', 2)
% plot([z(1,1) z(end,1)],-[lowTrough lowTrough], 'LineWidth', 2)
