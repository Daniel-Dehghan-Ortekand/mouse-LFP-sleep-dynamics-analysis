...
%
% INPUTS:
%   - thal_ch_norm_filtered_spi120808.mat: Pre-filtered LFP data.
%   - meansSignalForMethode2ToSeprate120808.mat: Population spindle signal.
%   - thal_spindles_120808_ch[X].mat: Detected spindle events per channel.
%   - real_peaks_120808_[X].mat: Peak amplitude indices per channel.
%
% OUTPUTS:
%   - sepratedSpindles120808.mat: Structure containing shank-specific 
...
% end
%    save thal_ch_norm_filtered_120808.mat thal_ch_norm_filtered -v7.3
...
%       save (['thal_spindles_120808_ch' num2str(k) '.mat'],'thal_spindles') 
% end

%% SECTION three : find pick
% for m=1:ch_number
%    load  (['thal_spindles_120808_ch' num2str(m) '.mat'])
...
% 
%    end
%     save (['real_peaks_120808_' num2str(m) '.mat'],'real_peaks') 
% 
% 
% end 
...
% hold on
% for i=1:ch_number
%    load  (['thal_spindles_120808_ch' num2str(i) '.mat'])
...
% load thal_ch_norm_filtered_120808.mat
% fs=1250;
...
% signal= Means;
% save meansSignalForMethode2ToSeprate120808    signal
% %
% figure
% hold on 
% for i=1:ch_number
...
load thal_ch_norm_filtered_spi120808.mat
load meansSignalForMethode2ToSeprate120808    signal
...
for i = 1:8
    ...
    for j = (i-1)*8+1:i*8
        ...
        load  (['thal_spindles_120808_ch' num2str(ch_ind) '.mat']);
...
refCH = load  (['thal_spindles_120808_ch' num2str(mostSpinCh) '.mat']);
...
for k=1:size(mostSpinForEachCh,2)
    load  (['thal_spindles_120808_ch' num2str(mostSpinForEachCh(1,k)) '.mat'])
    load meansSignalForMethode2ToSeprate120808    signal
...
%     MEAN = zeros(size(mostSpinForEachCh,2),size(thal_spindles,1));
    ...
    hist(MeAN,30)
    title('Histogram of distribution spindles event eachother ')
    xlabel('amplitude')
    hold on 
...
for k=1:size(mostSpinForEachCh,2)
    ...
    locspin=[];
    locPeaks =[];
load (['real_peaks_120808_' num2str(mostSpinForEachCh(1,k)) '.mat'])
load  (['thal_spindles_120808_ch' num2str(mostSpinForEachCh(1,k)) '.mat'])
...
sepratedSpindles120808.referencChannelForEachChannel = mostSpinForEachCh;

sepratedSpindles120808.referencChannel = mostSpinCh;
sepratedSpindles120808.localSPindles = LocSpinForEachRef;
sepratedSpindles120808.globalSPindles = globSpinForEachRef;
% sepratedSpindles120808.LocalPeaks = locPeaks;
% sepratedSpindles120808.globalPeaks = globPeaks;
% save sepratedSpindles120808 sepratedSpindles120808
%% save figures
% Get the path to the directory where you want to save the figures
save_path = 'D:/daniel/Mouse12-120808/SpindlesInf';
