%% Nearest-point matching between vectors
% This function finds the index of the value in vector Y that is closest to
% each value in vector X. It supports nearest, previous, and next matching
% modes and can optionally return the absolute and signed distances between
% matched points. The function handles unsorted input vectors and preserves
% the original ordering and dimensions of X, making it suitable for matching
% event times or other point-process data.



function [IND, D, RD] = nearestpoint(x,y,m) 
