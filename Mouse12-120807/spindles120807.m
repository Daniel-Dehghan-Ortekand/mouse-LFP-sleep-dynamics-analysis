% =========================================================================
% SCRIPT: Shank-Specific Sleep Spindle Detection & Global/Local Classification
% SUBJECT: Mouse12-120807
% =========================================================================
% DESCRIPTION:
% Processes multi-channel thalamic LFP data during SWS. Identifies the 
% optimal reference channel for each of the 8 probe shanks (groups of 8 
% channels). Computes a population-level spindle signal and uses its 
% amplitude to independently classify spindles as "Global" or "Local" 
% for each individual shank.
%
% INPUTS:
%   - thal_ch_norm_filtered_spi120807.mat: Pre-filtered LFP data.
%   - meansSignalForMethode2ToSeprate120807.mat: Population spindle signal.
%   - thal_spindles_120807_ch[X].mat: Detected spindle events per channel.
%   - real_peaks_120807_[X].mat: Peak amplitude indices per channel.
%
% OUTPUTS:
%   - sepratedSpindles120807.mat: Structure containing shank-specific 
%     reference channels and classified Global/Local spindles.
...
% end
%    save thal_ch_norm_filtered_120807.mat thal_ch_norm_filtered -v7.3
...
%       save (['thal_spindles_120807_ch' num2str(k) '.mat'],'thal_spindles') 
% end

%% SECTION three : find pick
% for m=1:ch_number
%    load  (['thal_spindles_120807_ch' num2str(m) '.mat'])
...
% 
%    end
%     save (['real_peaks_120807_' num2str(m) '.mat'],'real_peaks') 
% 
% 
% end 
...
% hold on
% for i=1:ch_number
%    load  (['thal_spindles_120807_ch' num2str(i) '.mat'])
...
% signal= Means;
% save meansSignalForMethode2ToSeprate120807    signal
% %
% figure
% hold on 
% for i=1:ch_number
...
load thal_ch_norm_filtered_spi120807.mat
load meansSignalForMethode2ToSeprate120807    signal

mostSpinForEachCh = zeros(2,8);
fs=1250;
for i = 1:8
    max_value = -inf;
    for j = (i-1)*8+1:i*8
        ...
        load  (['thal_spindles_120807_ch' num2str(ch_ind) '.mat']);
        if(max_value < size(thal_spindles,1))
            max_value = size(thal_spindles,1);
            mostSpinForEachCh(1,i) = ch_ind;
            mostSpinForEachCh(2,i) = max_value;
        end
    end
end
...
temp = (max(mostSpinForEachCh(2,:)));
temp2 = find(mostSpinForEachCh == temp)-1;
mostSpinCh = mostSpinForEachCh(temp2);
maxSpin = temp;

refCH = load  (['thal_spindles_120807_ch' num2str(mostSpinCh) '.mat']);
...
%%
MEAN = {};
for k=1:size(mostSpinForEachCh,2)
    load  (['thal_spindles_120807_ch' num2str(mostSpinForEachCh(1,k)) '.mat'])
    load meansSignalForMethode2ToSeprate120807    signal
    
    MeAN = zeros(1,size(thal_spindles,1));
%     MEAN = zeros(size(mostSpinForEachCh,2),size(thal_spindles,1));
    ...
    hist(MeAN,30)
    title('Histogram AVG signal between start and end spindles base on ref channel')
    xlabel('amplitude','Color','white')
    hold on 
    ...
    axis    square
    figure 
    plot((1:length(MeAN))/fs,MeAN,'LineWidth',2.5)
    title('averaged signal between start spindles and stop spindles ')
    axis square
end
%% seprate global and local base on threshold 
globSpinForEachRef = cell(2,size(mostSpinForEachCh,2));
LocSpinForEachRef = cell(2,size(mostSpinForEachCh,2));
for k=1:size(mostSpinForEachCh,2)
    globalspin =[];
    globPeaks = [];
    locspin=[];
    locPeaks =[];
load (['real_peaks_120807_' num2str(mostSpinForEachCh(1,k)) '.mat'])
load  (['thal_spindles_120807_ch' num2str(mostSpinForEachCh(1,k)) '.mat'])
...
globalspin(globrowsToDelete, :) = [];
globPeaks(:,globcolomnToDelete) = [];

globSpinForEachRef{1,k} = globalspin;
globSpinForEachRef{2,k} = globPeaks;
LocSpinForEachRef{1,k} = locspin;
LocSpinForEachRef{2,k} = locPeaks;
end 
% save spindles base on global and local 
sepratedSpindles120807.referencChannelForEachChannel = mostSpinForEachCh;

sepratedSpindles120807.referencChannel = mostSpinCh;
sepratedSpindles120807.localSPindles = LocSpinForEachRef;
sepratedSpindles120807.globalSPindles = globSpinForEachRef;
% sepratedSpindles120807.LocalPeaks = locPeaks;
% sepratedSpindles120807.globalPeaks = globPeaks;
% save sepratedSpindles120807 sepratedSpindles120807
%% save figures
% Get the path to the directory where you want to save the figures
save_path = 'D:/daniel/Mouse12-120807/SpindlesInf';
