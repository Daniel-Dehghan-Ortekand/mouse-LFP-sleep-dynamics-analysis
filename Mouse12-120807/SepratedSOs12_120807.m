% =========================================================================
% SCRIPT: Classify and Compare Slow Oscillation (SO) Waveforms by Amplitude Ratio
% SUBJECT: Mouse12-120807
% =========================================================================
% DESCRIPTION:
% Analyzes Slow Oscillations (SOs) by classifying them into two groups 
% ("Big" and "Short") based on the ratio of their secondary-to-primary 
% peak amplitudes. Computes the spatially averaged LFP waveforms time-locked 
% to these events, statistically compares their amplitudes using a t-test, 
% and evaluates SO rhythmicity by measuring the percentage of consecutive 
% events occurring within the characteristic peak-to-peak distance. 
% Additionally, it re-detects SOs using the Mölle algorithm to visualize 
% individual high-ratio events and their detection masks.
%
% INPUTS:
...
%   - SOdetectedForChooseOneChannelFromEachShank.mat: Previously detected SO 
...
% OUTPUTS:
%   - Visualizations: 
%       1. Continuous spatially averaged signals overlaid with raw LFP traces.
%       2. Time-locked average SO morphologies for "Big" vs "Short" groups.
%       3. Individual high-ratio SO events with SOMolle detection masks.
...
plot ((-fs/2:fs/2)/fs, means_allBig,Colors(1),"LineWidth",2 )
hold on 

plot ((-fs/2:fs/2)/fs ,means_allshort,Colors(2),'LineWidth',2 )
set(gca,'fontsize',16,'color',[0 0 0])
set(groot, 'defaultFigureColor', [0 0 0]);
set(groot, 'defaultAxesXColor', [1 1 1], 'defaultAxesYColor', [1 1 1], 'defaultAxesZColor', [1 1 1]); 
legend('bigger','shorter')
title('mean for shorter an bigger than thresold for mouse12-120807','Color','white' )
